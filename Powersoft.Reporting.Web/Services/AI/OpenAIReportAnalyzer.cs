...
public class OpenAIReportAnalyzer : IReportAnalyzer
{
    ...
    public async Task<AiChatReply> ChatAsync(List<AiChatMessage> history, string newUserMessage, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("Sending chat follow-up to OpenAI ({Model})", _options.Model);
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError("OpenAI chat error {Status}: {Body}", response.StatusCode, responseJson);
            throw new HttpRequestException($"OpenAI API returned {(int)response.StatusCode}: {ExtractErrorMessage(responseJson)}");
        }
        ...
        if (root.TryGetProperty("usage", out var usage))
        {
            ...
            if (usage.TryGetProperty("completion_tokens", out var ct2)) outTok = ct2.GetInt32();
        }

        _logger.LogInformation("OpenAI chat completed in {Ms}ms — {In}+{Out} tokens", sw.ElapsedMilliseconds, inTok, outTok);
